...
async function main() {
    ...
    if (embedding) {
        const results = await queryQdrant(embedding, 10); // Get top 5 results
        ...
        if (results && results.length > 0) {
            ...
            results.forEach((result, index) => {
                ...
                console.log('  Payload:');
                // Print relevant payload fields, e.g., text, cui
                if (result.payload) {
                    console.log(`    Text: ${result.payload.text || 'N/A'}`)
                    console.log(`    CUI: ${result.payload.cui || 'N/A'}`)
                    console.log(`    Language: ${result.payload.language || 'N/A'}`)
                    console.log(`    Source: ${result.payload.sourceAbbreviation || 'N/A'}`)
